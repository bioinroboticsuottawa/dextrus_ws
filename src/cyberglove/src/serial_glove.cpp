...
CybergloveSerial::CybergloveSerial(std::string serial_port,
		std::string cyberglove_version, std::string streaming_protocol,
	...
	cereal_port = boost::shared_ptr<cereal::CerealPort>(
			new cereal::CerealPort());
...
int CybergloveSerial::start_stream() {
	...
	cereal_port->startReadStream(
			boost::bind(&CybergloveSerial::stream_callback, this, _1, _2));
			...
			case reception_16bit::TIMESTAMP:
				...
				if (timestamp_bytes_ == timestamp_size) {
					if (current_value == 'S') {
						...
						std::cout << "Sync error. Not an S: Reset frame"
								<< std::endl;
						reception_state_ = reception_16bit::SYNCHRONIZATION_1;
					}
				}
...
//              std::cout << aux << std::endl;
					if (sensor_value_ > 0x0FFF) {
						...
						std::cout << "bad sensor value: " << aux
								<< " Reset frame" << std::endl;
						reception_state_ = reception_16bit::SYNCHRONIZATION_1;
						break;
					}

					glove_positions[glove_pos_index] = (((float) sensor_value_)
							- 1.0f) / (float) (0x0FFF - 1);
					++glove_pos_index;
					byte_index_ = 0;
				} else {
...
//						callback_function(glove_positions, light_on);
						if (current_value == 10 && no_errors)
							callback_function(glove_positions, light_on);
						if (current_value != 10)
							std::cout << "Last char is not 10: "
									<< current_value << std::endl;
					} else if (cyberglove_version_ == "2") {
						//the last char of the line should be 0
						//if it is 0, then the full message has been received,
						...
						if (current_value == 0 && no_errors)
							callback_function(glove_positions, light_on);
						if (current_value != 0)
							std::cout << "Last char is not 0: " << current_value
									<< std::endl;
					}
					reception_state_ = INITIAL;
					break;
				...
				default:
					if (cyberglove_version_ == "1") {
						if (current_value == 32) {
							if (v1_current != "") {
//								v1_reads.push_back(v1_current);
//								printf("v1_current %s\n", v1_current.c_str());
								char* pEnd;
								glove_positions[glove_pos_index] =
										(((float) std::strtof(
												v1_current.c_str(), &pEnd))
												- 1.0f) / 254.0f;
								v1_current = "";
								break;
							}
						...
						v1_current.push_back((char) current_value);
						glove_pos_index--;
						break;
					}
					...
					//   -> we convert them to float in the range [0;1]
					glove_positions[glove_pos_index] = (((float) current_value)
							- 1.0f) / 254.0f;
					break;
				}
				++glove_pos_index;
